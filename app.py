...
def sentiment_generator():
    df = pd.read_csv('manufacturing-tweets.csv')
    tweets = df[["user","full_text"]]
    ...
    while n < len(tweets):
        ...
        tweet_list.append(tweets[:n])

        # Loop through tweets
        for tweet in tweets['full_text'][:n]:
            analysis = TextBlob(tweet)
            score = SentimentIntensityAnalyzer().polarity_scores(tweet)
            polarity += analysis.sentiment.polarity # running cumulative score
            
            if score["neg"] > score["pos"]:  # tweet predominantly negative
                negative += 1
            elif score["neg"] < score["pos"]:  # tweet predominantly negative
                positive += 1
            if score["neg"] == score["pos"]:  # tweet predominantly negative
                neutral += 1
        
        # for item in tweet_list[-5:]:
        #     print(item)
        #     print("\n")

        print(tweet_list)
        
        update_news(tweet_list[-5:])

        time.sleep(10)

# API Call to update news
def update_news(tweet_list):
                        ...
                        children=[
                            ...
                            html.Td(
                                children=[
                                    html.P(
                                        className="td-link",
                                        children=tweet_list[i]["full_text"],
                                    )
                                ]
                            )
                        ]
    ...
    children=[
        ...
        html.Div(
            ...
            children=[
                ...
                html.Div(
                    className="div-news",
                    children=[html.Div(id="news", children=sentiment_generator())],
                ),
            ],
        ),
        # Right Panel Div
